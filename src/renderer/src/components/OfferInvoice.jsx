...
import { deletePaymentOffer, getPaymentsOffer } from '../redux/api/paymentOfferApi'
const OfferInvoice = () => {
  ...
  const filteredPaymentsOffer = useMemo(() => {
    return paymentsOffer?.filter((c) => c.id.toString().includes(searchItem))
  }, [searchItem, paymentsOffer])

  const { totalFilteredPaymentsOffer, pages } = useMemo(() => {
    const filteredPaymentsOffer = paymentsOffer?.filter((c) => c.id.toString().includes(searchItem))

    const totalFilteredPaymentsOffer = filteredPaymentsOffer?.length
    const pages = Math.ceil(totalFilteredPaymentsOffer / rowsPerPage)

    return { totalFilteredPaymentsOffer, pages, filteredPaymentsOffer }
  }, [searchItem, paymentsOffer, rowsPerPage])

  const items = useMemo(() => {
    const start = (page - 1) * rowsPerPage
    const end = start + rowsPerPage
    return filteredPaymentsOffer?.slice(start, end)
  }, [page, filteredPaymentsOffer, rowsPerPage])
  ...
  return (
    <>
      ...
      {paymentsOffer && items && (
        <Table
          items={items}
          totale={totalFilteredPaymentsOffer}
