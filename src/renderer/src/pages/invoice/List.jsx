...
const List = () => {
  ...
  const { loadingGet, error } = useSelector((state) => state.category)
  ...
  return (
    <section className="w-full flex flex-col gap-3">
      ...
      {Payments && items && (
        <Table items={items} totale={totalFilteredPayments} setItemToDelete={setItemToDelete} />
      )}
      {loadingGet && (
            ...
            {items.length > 0 ? (
              items.map((f) => (
                <tr className="hover:bg-blue-200 dark:hover:bg-gray-900" key={f.id}>
                  ...
                  <td className="whitespace-nowrap px-2 py-1 font-medium text-gray-900 dark:text-white w-auto text-start ">
                    <div className="">{formatMoney(f.totalPrice)}</div>
                  </td>
                
                  <td className="whitespace-nowrap px-2 py-1 font-medium text-gray-900 dark:text-white w-auto text-center">
                    <Chip
                      startContent={f.isLocal ? <MdOutlineShop2 size={18} /> : <GiShop size={18} />}
                      variant="faded"
                      color={f.isLocal ? 'success' : 'danger'}
                    >
                      <span className="font-semibold">{f.isLocal ? 'onligne' : 'offline'}</span>
                    </Chip>
                  </td>
                  <td className="whitespace-nowrap px-2 py-1 font-medium text-gray-900 dark:text-white w-auto text-center">
                    <Chip
